...
export async function postInstall(
  ...
  try {
    ...
    core.warning(`Prewarm or selftest failed: ${err}`)
  }
}
...
export async function run(
  ...
  try {
    ...
    core.debug('Resolving effective options')
    const effectiveOptions: ElideSetupActionOptions = options
      ...
      : buildOptions({
          ...
          os: normalizeOs(
            stringOption(OptionName.OS, process.platform) || 'linux'
          ),
          arch: normalizeArch(
            stringOption(OptionName.ARCH, process.arch) || 'amd64'
          ),
    ...
    if (!effectiveOptions.force) {
      ...
      try {
        existing = await io.which('elide', true)
      } catch (err) {
