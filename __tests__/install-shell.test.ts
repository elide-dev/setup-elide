...
const { installViaShell } = await import('../src/install-shell')
...
describe('install-shell', () => {
  ...
  describe('bash (linux/macOS)', () => {
    it('should download and execute the bash install script with --gha', async () => {
      const options = buildOptions({
        os: 'darwin',
        arch: 'aarch64',
        version: 'latest'
      })
      const result = await installViaShell(options)
  ...
  describe('powershell (windows)', () => {
    it('should download and execute the PowerShell script with -Gha', async () => {
      downloadToolMock.mockResolvedValue('/tmp/install.ps1')
      const options = buildOptions({
        os: 'windows',
        arch: 'amd64',
        version: 'latest'
      ...
      expect(downloadToolMock).toHaveBeenCalledWith(
        'https://dl.elide.dev/cli/install.ps1'
      )
